...
## Формат
Возвращай ответы ТОЛЬКО в формате JSON, без markdown-обёрток, без комментариев.`;
...
function syllableCountGeneratePrompt(topic: string, difficulty: number): string {
  ...
  return `Создай упражнение-дрель типа "Силлабический счётчик".
...
function stressPatternGeneratePrompt(topic: string, difficulty: number): string {
  ...
  return `Создай упражнение-дрель типа "Определение ударений".
...
- хорей: ТА-ta-ТА-ta-TA-ta
- дактиль: ТА-ta-ta-TA-ta-ta
- амфибрахий: ta-TA-ta-ta-TA-ta
- анапест: ta-ta-TA-ta-ta-TA
...
function rhymeMatchGeneratePrompt(topic: string, difficulty: number): string {
  ...
  return `Создай упражнение-дрель типа "Подбор рифмы".
...
function lineBuilderGeneratePrompt(topic: string, difficulty: number): string {
  return `Создай упражнение-дрель типа "Конструктор строки".
...
## Формат ответа (JSON)
{
  ...
  "hints": ["Пройдись по вариантам, отбивая ритм: та-ТА-ta-TA..."]
}`;
}
...
export function getGeneratePrompt(type: ExerciseType, topic: string, difficulty: number): string {
  return GENERATE_PROMPTS[type](topic, difficulty);
}
